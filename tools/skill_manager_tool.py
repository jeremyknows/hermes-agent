...
def _surface_mutation_kind(action: str) -> str:
    if action == "create":
        return "skill_create"
    if action == "delete":
        return "skill_delete"
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            ...
            pass
        try:
            _emit_agent_surface_mutated(action, name, result)
        except Exception:
            logger.debug("agent_surface_mutated emit failed for skill %s", name, exc_info=True)
